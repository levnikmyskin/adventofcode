...
use crate::similar_boxes::get_similar_boxes_indexes;

pub const INP: [&'static str; 250] = [
...
fn main() {
    let (first, second, index) = get_similar_boxes_indexes().unwrap();
    let first = INP[first];
    let second = INP[second];
    let first_part = &first[0..index];
    let second_part = &second[index+1..];
    println!("Characters in common: {}{}", first_part, second_part);
}
